...
export async function canViewLesson(
  { role, userId }: { userId: string | undefined; role: UserRole | undefined },
  lesson: { id: string; status: LessonStatus }
) {
  "use cache";
  if (role == "admin" || lesson.status === "preview") return true;
  if (userId == null || lesson.status === "private") return true;
  ...
  const [data] = await db
    ...
    .leftJoin(
      LessonTable,
      and(eq(LessonTable.sectionId, CourseTable.id), wherePublicLessons)
    )
    .where(
      and(
